...
public class VersionChecker implements Listener{
	 ...
	 */
	public static VersionChecker start(final double version, final ActionAlert instance){
		...
		new BukkitRunnable() {
			...
			public void run() {
				try {
					...
					conn.addRequestProperty("User-Agent", "ActionAlert version check");
					...
					if(toCheck > version){
						...
						instance.getLogger().info("A new version of ActionAlert is available: "+versionChecker.newVersion);
					}else
	...
	public void onPlayerJoinEvent(final PlayerJoinEvent e){
		if(!e.getPlayer().hasPermission("actionalert.versioncheck"))
			return;
