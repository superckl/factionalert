...
public class AlertGroupStorage {
	...
	public static void readExcludes(){
		for(val name:AlertGroupStorage.storage.keySet()){
			val toRead = new File(ActionAlert.getInstance().getDataFolder(), new StringBuilder("data/excludes_").append(name).append(".yml").toString());
			if(!toRead.exists())
				continue;
	...
	public static void saveExcludes() throws IOException{
		for(val name:AlertGroupStorage.storage.keySet()){
			val toSave = new File(ActionAlert.getInstance().getDataFolder(), new StringBuilder("data/excludes_").append(name).append(".yml").toString());
